...
class helper
{
    ...
    public static function createorder($vendor, $user_id, $session_id, $payment_type_data, $payment_id, $customer_email, $customer_name, $customer_mobile, $stripeToken, $grand_total, $tips, $delivery_area, $delivery_charge, $address, $building, $landmark, $postal_code, $discount_amount, $sub_total, $tax, $tax_name, $delivery_time, $delivery_date, $couponcode, $order_type, $notes, $filename, $table, $tablename, $buynow)
    {
        try {
            ...
            if ($data->count() > 0) {
                ...
                $orderCount = Order::where('vendor_id', $vendor)->count();
                if ($orderCount == 1) {
                    $firstSaleAutomation = Automation::where('trigger_type', AutomationTriggerType::FIRST_SALE->value)->where('is_active', true)->first();
                    if ($firstSaleAutomation) {
                        $alreadySent = AutomationLog::where('user_id', $vendorinfo->id)
            ...
            Log::error('createorder helper error: '.$th->getMessage().' at line '.$th->getLine().' in '.$th->getFile());
    ...
    public static function vendor_register($vendor_name, $vendor_email, $vendor_mobile, $vendor_password, $firebasetoken, $slug, $google_id, $facebook_id, $country_id, $city_id, $store, $product_type)
    {
        try {
            ...
            $data->firebase = '-';
            $storeCategory = StoreCategory::find($store);
            ...
            $storeCreatedAutomation = Automation::where('trigger_type', AutomationTriggerType::STORE_CREATED->value)->where('is_active', true)->first();
    ...
    public static function create_order_invoice($customer_email, $customer_name, $companyemail, $companyname, $vendorid, $order_number, $order_type, $delivery_date, $delivery_time, $grand_total, $trackurl)
    {
        ...
        try {
            ...
            Log::error('OrderInvoice Mail Error: '.$th->getMessage(), ['exception' => $th]);
    ...
    public static function getallpayment($vendor_id)
    {
        if (Auth::user() && Auth::user()->type == 3) {
            ...
            $payment = Payment::where('is_available', '1')->whereNotIn('payment_type', [16])->where('vendor_id', $vendor_id)->where('is_activate', 1)->orderBy('reorder_id')->get();
        }
    ...
    public static function imageresize($file, $directory_name)
    {
        ...
        } elseif (Str::endsWith($reimage, '.jpg')) {
            $convert = $convert->toJpeg();
        } elseif (Str::endsWith($reimage, '.webp')) {
            $convert = $convert->toWebp();
        } elseif (Str::endsWith($reimage, '.gif')) {
            $convert = $convert->toGif();
        } elseif (Str::endsWith($reimage, '.png')) {
            $convert = $convert->toPng();
        } elseif (Str::endsWith($reimage, '.avif')) {
            $convert = $convert->toAvif();
        } elseif (Str::endsWith($reimage, '.bmp')) {
    ...
    public static function is_restaurant_store($vendor_id): bool
    {
        ...
        $storeCategory = StoreCategory::find($user->store_id);
        if (! $storeCategory) {
            return false;
        }
