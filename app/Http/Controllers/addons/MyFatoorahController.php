...
class MyFatoorahController extends Controller
{
    ...
    public function myfatoorahrequest(Request $request)
    {
        try {
            ...
            $getkey = Payment::select('environment', 'secret_key')->where('payment_type', $request->payment_type)->where('vendor_id', '1')->first();
    ...
    public function front_myfatoorahrequest(Request $request)
    {
        try {
            ...
            $getkey = Payment::select('environment', 'secret_key', 'currency')->where('payment_type',  $request->payment_type)->where('vendor_id', $request->vendor_id)->first();
            if ($getkey->environment == 0) {
                $mode = false;
            } else {
                $mode = true;
    ...
    public function myfatoorahapi(Request $request)
    {
        try {
            Session::put('customer_name', $request->customer_name);
            Session::put('customer_email', $request->customer_email);
            Session::put('vendor_id', $request->vendor_id);
            Session::put('grand_total', $request->grand_total);
            Session::put('successurl', "https://www.google.com/");
            Session::put('failureurl', "https://www.facebook.com/");
            $getkey = Payment::select('environment', 'secret_key', 'currency')->where('payment_type',  '9')->where('vendor_id', $request->vendor_id)->first();
            ...
            $this->mfObj = new PaymentMyfatoorahApiV2($getkey->secret_key, $getkey->currency, $mode);
            $paymentMethodId = 0; // 0 for MyFatoorah invoice or 1 for Knet in test mode
            $data            = $this->mfObj->getInvoiceURL($this->api_getPayLoadData(), $paymentMethodId);
            return response()->json(['status' => 1, 'message' => trans('messages.success'), 'url' => $data['invoiceURL'], 'successurl' => session()->get('successurl'), 'failureurl' => session()->get('failureurl')], 200);
        } catch (\Exception $e) {
            return response()->json(['status' => 0, 'message' => $e->getMessage()], 200);
        }
    }
     ...
     * @return array
     */
    private function api_getPayLoadData($orderId = null)
    {
        return [
            'CustomerName'       => Session::get('customer_name'),
            'InvoiceValue'       => session()->get('grand_total'),
