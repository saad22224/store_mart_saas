    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb m-0">
                <li class="breadcrumb-item text-dark"><a href="{{ URL::to('admin/categories') }}" class="color-changer">{{ trans('labels.category')
                        }}</a></li>
                    ...
                    <form action="{{ URL::to('admin/categories/update-' . $editcategory->slug) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                @if(!empty($editcategory->image) && $editcategory->image != 'default.png')
                                    <img src="{{ helper::image_path($editcategory->image) }}" class="img-fluid rounded mt-2" style="height: 50px">
                                @endif
                            </div>
                            <div class="mt-3 {{ session()->get('direction') == '2' ? 'text-start' : 'text-end' }}">
                                <a href="{{ URL::to('admin/categories') }}" class="btn btn-danger px-sm-4">{{
                                    trans('labels.cancel') }}</a>
                                <button class="btn btn-primary px-sm-4 {{Auth::user()->type == 4 ? (helper::check_access('role_categories', Auth::user()->role_id, $vendor_id, 'edit') == 1  ? '':'d-none'): ''}}" @if (env('Environment') == 'sendbox') type="button"
