                    ...
                    <form action="{{ URL::to('admin/cities/update-' . $editcity->id) }}" method="POST"
                        ...
                        <div class="row">
                                ...
                                <select name="country" class="form-select">
                                    ...
                                    @foreach ($allcountry as $country)
                                        <option value="{{ $country->id }}"
                                            {{ $country->id == $editcity->country_id ? 'selected' : '' }}>
                                            {{ $country->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="name" value="{{ $editcity->city }}"
                            ...
                            <div class="mt-3 {{ session()->get('direction') == '2' ? 'text-start' : 'text-end' }}">
                                ...
                                <button
                                    class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_areas', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
