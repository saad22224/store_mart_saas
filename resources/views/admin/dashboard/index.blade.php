                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <h3 class="dash-stat-value mb-0" style="font-size:1.1rem">
                                    @if (!empty($currentplanname)) {{ @$currentplanname->name }} @else - @endif
                                </h3>
                            </div>
                            <div class="dash-stat-icon"><i class="fa-regular fa-medal"></i></div>
                        </div>
                ...
                <div class="card-body p-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <h3 class="dash-stat-value mb-0" style="font-size:1.2rem">{{ helper::currency_formate($totalrevenue, $vendor_id) }}</h3>
                        </div>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                            <div class="d-flex align-items-center gap-3">
                                <img src="{{ helper::image_path(@Auth::user()->image) }}" class="dash-welcome-avatar" alt="">
                                <div>
                                    ...
                                    <div class="dropdown" id="tour-quick-add">
                                        ...
                                        <ul class="dropdown-menu fw-500 fs-7 text-dark" style="border-radius:12px;overflow:hidden;border:none;box-shadow:0 10px 40px rgba(0,0,0,0.15)">
                                            <li><a class="dropdown-item py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_products') == 1 ? 'd-block' : 'd-none' }}" href="{{ URL::to('/admin/products') }}"><i class="fa-solid fa-box me-2 text-muted"></i>{{ trans('labels.products') }}</a></li>
                                            <li><a class="dropdown-item py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_categories') == 1 ? 'd-block' : 'd-none' }}" href="{{ URL::to('/admin/categories') }}"><i class="fa-solid fa-layer-group me-2 text-muted"></i>{{ trans('labels.categories') }}</a></li>
                                            <li><a class="dropdown-item py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_settings') == 1 ? 'd-block' : 'd-none' }}" href="{{ URL::to('/admin/basic_settings') }}"><i class="fa-solid fa-gear me-2 text-muted"></i>{{ trans('labels.basic_settings') }}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                <div class="dash-qr">
                                    <img src="https://qrcode.tec-it.com/API/QRCode?data={{ $url }}&choe=UTF-8" class="object" style="width:90px;height:90px" alt="">
                                </div>
            ...
            <div class="card dash-chart-card h-100">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-3 pb-3 border-bottom justify-content-between">
                        ...
                        <select class="form-select form-select-sm w-auto" id="revenueyear" data-url="{{ URL::to('/admin/dashboard') }}" style="border-radius:10px">
                            @if (count($revenue_years) > 0 && !in_array(date('Y'), array_column($revenue_years->toArray(), 'year')))
                                <option value="{{ date('Y') }}" selected>{{ date('Y') }}</option>
                            @endif
                            @forelse ($revenue_years as $revenue)
                                <option value="{{ $revenue->year }}" {{ date('Y') == $revenue->year ? 'selected' : '' }}>{{ $revenue->year }}</option>
                            @empty
            ...
            <div class="card dash-chart-card h-100">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center justify-content-between mb-3 pb-3 border-bottom">
                        ...
                        <select class="form-select form-select-sm w-auto" id="doughnutyear" data-url="{{ request()->url() }}" style="border-radius:10px">
        ...
        <div class="row g-3 mb-4">
            <div class="col-xl-6" id="tour-products">
                <div class="card dash-table-card h-100">
                    <div class="card-body p-4">
                        <h5 class="dash-chart-title pb-3 color-changer border-bottom">{{ trans('labels.top_products') }}</h5>
                                    ...
                                    @if (count($topitems) > 0)
                                        @foreach (@$topitems as $row)
                                            <tr class="fs-7 fw-500 text-dark align-middle">
                                                <td><img src="{{ Helper::image_path($row['product_image']->image) }}" class="rounded hw-50 object" alt="" style="border-radius:10px !important"></td>
                                                <td><a href="{{ URL::to('admin/products/edit-' . $row->slug) }}" class="td_a">{{ $row->item_name }}</a></td>
                                                <td>{{ @$row['category_info']->name }}</td>
                                                <td>
                                                    @php $per = $getorderdetailscount > 0 ? ($row->item_order_counter * 100) / $getorderdetailscount : 0; @endphp
                                                    {{ number_format($per, 2) }}%
                                                    <div class="progress h-10-px" style="border-radius:10px"><div class="progress-bar {{ $ran[array_rand($ran, 1)] }}" style="width:{{ $per }}%;border-radius:10px" role="progressbar"><span class="sr-only">{{ $per }}%</span></div></div>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
            ...
            <div class="col-xl-6">
                <div class="card dash-table-card h-100">
                    <div class="card-body p-4">
                        <h5 class="dash-chart-title pb-3 color-changer border-bottom">{{ trans('labels.top_customers') }}</h5>
                        <div class="table-responsive">
                            <table class="table">
                                <thead><tr>
                                    <th class="fs-15 fw-500">{{ trans('labels.image') }}</th>
                                ...
                                <tbody>
                                    ...
                                    @if (count($topusers) > 0)
                                        @foreach (@$topusers as $user)
                                            <tr class="fs-7 fw-500 text-dark align-middle">
                                                <td><img src="{{ Helper::image_path($user->profile_image) }}" class="rounded hw-50 object" alt="" style="border-radius:10px !important"></td>
                                                <td><div class="fs-7 fw-500 td_a"><p>{{ $user->name }}</p><p>{{ $user->mobile }}</p></div></td>
                                                <td>{{ $user->email }}</td>
                                                <td>
                                                    {{ number_format($per, 2) }}%
                                                    <div class="progress h-10-px" style="border-radius:10px"><div class="progress-bar {{ $ran[array_rand($ran, 1)] }}" style="width:{{ $per }}%;border-radius:10px" role="progressbar"><span class="sr-only">{{ $per }}%</span></div></div>
