                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="d-flex flex-wrap gap-2 mt-2" id="tour-quick-add">
                                        <a class="btn btn-sm px-3 py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_products') == 1 ? 'd-flex' : 'd-none' }} align-items-center"
                                            ...
                                            <i class="fa-solid fa-box me-2"></i> {{ trans('labels.products') }}
                                        </a>
                                        <a class="btn btn-sm px-3 py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_categories') == 1 ? 'd-flex' : 'd-none' }} align-items-center"
                                            href="{{ URL::to('/admin/categories') }}"
                                            style="background:#fff;color:var(--bs-primary);border-radius:8px;font-weight:700;font-size:.85rem;border:none;box-shadow:0 4px 10px rgba(0,0,0,0.1); transition: all 0.3s ease;"
                                            onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 15px rgba(0,0,0,0.15)';"
                                            onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 10px rgba(0,0,0,0.1)';">
                                            <i class="fa-solid fa-layer-group me-2"></i> {{ trans('labels.categories') }}
                                        </a>
                                        <a class="btn btn-sm px-3 py-2 {{ helper::check_menu(@Auth::user()->role_id, 'role_settings') == 1 ? 'd-flex' : 'd-none' }} align-items-center"
                                            href="{{ URL::to('/admin/basic_settings') }}"
                                            style="background:#fff;color:var(--bs-primary);border-radius:8px;font-weight:700;font-size:.85rem;border:none;box-shadow:0 4px 10px rgba(0,0,0,0.1); transition: all 0.3s ease;"
                                            onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 15px rgba(0,0,0,0.15)';"
                                            onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 10px rgba(0,0,0,0.1)';">
                                            <i class="fa-solid fa-gear me-2"></i> {{ trans('labels.basic_settings') }}
                                        </a>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            (function() {
                const steps = [{
                        ...
                        desc: 'من هنا يمكنك الاطلاع على إحصائيات متجرك، بما في ذلك عدد المنتجات والطلبات والإيرادات.',
                        position: 'bottom'
                    },
                    {
                        el: '#tour-quick-add',
                        title: 'إضافة سريعة',
                        desc: 'من هنا يمكنك إضافة المنتجات، والأقسام، والإعدادات بسرعة وسهولة دون الحاجة للبحث في القوائم.',
                        position: 'bottom'
                    },
                    {
                        el: '#tour-chart',
                        title: 'تقارير الإيرادات',
                        desc: 'رسم بياني يوضح إيرادات متجرك على مدار العام، مع إمكانية اختيار وعرض بيانات سنوات مختلفة.',
                        position: 'bottom'
                    },
                    {
                        el: '#tour-orders-table',
                        title: 'الطلبات الجارية',
                        desc: 'جدول يعرض الطلبات التي تتطلب المتابعة، حيث يمكنك تغيير حالة الطلب مباشرة من هنا.',
                        position: 'top'
                    },
                    {
                        el: '.sidebar',
                        title: 'القائمة الجانبية',
                        desc: 'تتيح لك القائمة الجانبية التنقل بسهولة بين جميع أقسام لوحة التحكم المختلفة.',
                        position: 'right'
                    }
                ];
                let idx = 0;
