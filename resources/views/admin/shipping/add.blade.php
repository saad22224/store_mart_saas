                    ...
                    <form action="{{ URL::to('/admin/shipping/save') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mt-3 d-flex gap-1 justify-content-end">
                            ...
                            <button
                                ...
                                class="btn btn-primary px-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                        </div>
                    </form>
                </div>
            </div>
