...
<style>
    ...
    .t16-search-icon {
        position: absolute;
        top: 50%;
        {{ session()->get('direction') == 2 ? 'right: 18px;' : 'left: 18px;' }}
    ...
    .t16-item-badge {
        position: absolute;
        top: 12px;
        {{ session()->get('direction') == 2 ? 'left: 12px;' : 'right: 12px;' }}
...
<div class="t16-search-bar">
    <div class="t16-search-wrap">
        ...
        <input type="text"
               ...
               id="t16SearchInput"
               placeholder="{{ trans('labels.search') == 'labels.search' ? 'Search for categories, products...' : trans('labels.search') }}"
               autocomplete="off">
    </div>
</div>
...
<div class="t16-cat-header">
    <a href="{{ URL::to($storeinfo->slug) }}" class="t16-back-btn">
        <i class="fa-solid fa-arrow-left"></i>
        {{ trans('labels.back') == 'labels.back' ? 'Back' : trans('labels.back') }}
    </a>
    <h1 class="t16-cat-name">{{ $category->name }}</h1>
</div>
    ...
    <div class="t16-cat-grid" id="t16ProductsGrid">
        @foreach($products as $index => $product)
            ...
            <a href="{{ URL::to($storeinfo->slug.'/detail-'.$product->slug) }}"
               ...
               style="animation-delay: {{ ($index % 9) * 0.06 }}s;">
                <div class="t16-item-card-img">
                    <span class="t16-item-badge">{{ $category->name }}</span>
                    <img src="{{ $imgSrc }}"
                         alt="{{ $product->item_name }}"
                         loading="lazy">
                </div>
                <div class="t16-item-card-body">
                    <div class="t16-item-card-name">{{ $product->item_name }}</div>
                    <div class="t16-item-card-price">
                        {{ helper::currency_formate($price, $storeinfo->id, $product->currency) }}
                        @if($off > 0)
                            <span class="t16-item-card-old-price">
                                {{ helper::currency_formate($original_price, $storeinfo->id, $product->currency) }}
                            </span>
                        @endif
                    </div>
                </div>
        ...
        {{-- No results message (shown by search JS) --}}
        <div class="t16-no-results" id="t16NoResults">
            ...
            {{ trans('labels.no_product_found') == 'labels.no_product_found' ? 'لا توجد منتجات مطابقة' : trans('labels.no_product_found') }}
        </div>
    </div>
