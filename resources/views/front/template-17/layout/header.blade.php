...
<header class="vela-header">
    <div class="vela-header-inner">
            ...
            <div class="vela-header-left">
                ...
                <div class="dropdown vela-language-wrap">
                    ...
                    <ul class="dropdown-menu">
                        @if(\App::getLocale() == 'en')
                            <li><a class="dropdown-item" href="{{ URL::to('/lang/change?lang=ar') }}">AR</a></li>
                        @else
                            <li><a class="dropdown-item" href="{{ URL::to('/lang/change?lang=en') }}">EN</a></li>
                        @endif
                    </ul>
                </div>
            </div>

            <a href="{{ URL::to($storeinfo->slug) }}" class="vela-logo" aria-label="{{ $storeinfo->name }}">
                <img src="{{ helper::image_path(helper::appdata(@$storeinfo->id)->logo) }}" alt="{{ $storeinfo->name }}">
            </a>

            <div class="vela-header-right">
                <a href="{{ URL::to($storeinfo->slug . '/search') }}" class="vela-header-icon" aria-label="{{ trans('labels.search') }}">
                    <i class="fa-solid fa-magnifying-glass" style="font-size: 18px;"></i>
                </a>

                @if (helper::appdata(@$storeinfo->id)->online_order == 1)
                    <a href="{{ URL::to($storeinfo->slug . '/cart/') }}" class="vela-header-icon" aria-label="{{ trans('labels.cart') }}">
                          <i class="fa-solid fa-cart-shopping" style="font-size: 18px;"></i>
                          <span class="vela-cart-badge" id="cartcnt">{{ $cartCount }}</span>
                    </a>
                @endif
            </div>
        </div>
        ...
        <nav class="vela-main-nav" aria-label="Categories">
            @foreach($template17Categories as $cat)
                <a href="{{ URL::to(@$storeinfo->slug.'/category/'.$cat->slug) }}" class="vela-nav-link {{ request()->is($storeinfo->slug.'/category/'.$cat->slug) ? 'active' : '' }}">{{ $cat->name }}</a>
            @endforeach
        </nav>
    </div>
</header>
...
<div class="offcanvas offcanvas-start vela-mobile-nav" tabindex="-1" id="vela-mobile-nav" aria-labelledby="vela-mobile-nav-title">
    <div class="offcanvas-header border-bottom">
        <a href="{{ URL::to($storeinfo->slug) }}" class="vela-logo" id="vela-mobile-nav-title">
            <img src="{{ helper::image_path(helper::appdata(@$storeinfo->id)->logo) }}" alt="{{ $storeinfo->name }}">
        </a>
