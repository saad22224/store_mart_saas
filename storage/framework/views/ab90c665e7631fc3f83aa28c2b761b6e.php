<?php $__env->startSection('content'); ?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if(Auth::user()->type == 2 || (Auth::user()->type == 4 && Auth::user()->vendor_id != 1)): ?>
            <button class="btn btn-sm px-3 py-2" id="startTourBtn" onclick="startTour()" style="background:linear-gradient(135deg,var(--bs-primary),var(--bs-secondary));color:#fff;border-radius:12px;font-weight:700;font-size:.82rem">
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <h3 class="dash-stat-value mb-0" style="font-size:1.1rem">
                                    <?php if(!empty($currentplanname)): ?> <?php echo e(@$currentplanname->name); ?> <?php else: ?> - <?php endif; ?>
                                </h3>
                            </div>
                            <div class="dash-stat-icon"><i class="fa-regular fa-medal"></i></div>
                        </div>
                ...
                <div class="card-body p-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <h3 class="dash-stat-value mb-0" style="font-size:1.2rem"><?php echo e(helper::currency_formate($totalrevenue, $vendor_id)); ?></h3>
                        </div>
    ...
    <?php if(Auth::user()->type == 2 || (Auth::user()->type == 4 && Auth::user()->vendor_id != 1)): ?>
        <?php
            if ($user->custom_domain == null) { $url = URL::to('/' . $user->slug); } else { $url = 'https://' . $user->custom_domain; }
        ?>
        <div class="row mb-4">
            <div class="col-12">
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                            <div class="d-flex align-items-center gap-3">
                                <img src="<?php echo e(helper::image_path(@Auth::user()->image)); ?>" class="dash-welcome-avatar" alt="">
                                <div>
                                    ...
                                    <div class="dropdown" id="tour-quick-add">
                                        <a class="btn btn-sm px-3 py-1 dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" style="background:#fff;color:var(--bs-primary);border-radius:8px;font-weight:700;font-size:.85rem;border:none;box-shadow:0 4px 10px rgba(0,0,0,0.1)">
                                            <i class="fa-regular fa-plus me-1"></i> <?php echo e(trans('labels.quick_add')); ?>

                                        </a>
                                        <ul class="dropdown-menu fw-500 fs-7 text-dark" style="border-radius:12px;overflow:hidden;border:none;box-shadow:0 10px 40px rgba(0,0,0,0.15)">
                                            <li><a class="dropdown-item py-2 <?php echo e(helper::check_menu(@Auth::user()->role_id, 'role_products') == 1 ? 'd-block' : 'd-none'); ?>" href="<?php echo e(URL::to('/admin/products')); ?>"><i class="fa-solid fa-box me-2 text-muted"></i><?php echo e(trans('labels.products')); ?></a></li>
                                            <li><a class="dropdown-item py-2 <?php echo e(helper::check_menu(@Auth::user()->role_id, 'role_categories') == 1 ? 'd-block' : 'd-none'); ?>" href="<?php echo e(URL::to('/admin/categories')); ?>"><i class="fa-solid fa-layer-group me-2 text-muted"></i><?php echo e(trans('labels.categories')); ?></a></li>
                                            <li><a class="dropdown-item py-2 <?php echo e(helper::check_menu(@Auth::user()->role_id, 'role_settings') == 1 ? 'd-block' : 'd-none'); ?>" href="<?php echo e(URL::to('/admin/basic_settings')); ?>"><i class="fa-solid fa-gear me-2 text-muted"></i><?php echo e(trans('labels.basic_settings')); ?></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                <div class="dash-qr">
                                    <img src="https://qrcode.tec-it.com/API/QRCode?data=<?php echo e($url); ?>&choe=UTF-8" class="object" style="width:90px;height:90px" alt="">
                                </div>
                                <button class="btn btn-sm px-3 py-2" id="copyButton" style="background:rgba(255,255,255,0.15);color:#fff;border-radius:10px;font-weight:600;font-size:.82rem;backdrop-filter:blur(10px)">
            ...
            <div class="card dash-chart-card h-100">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-3 pb-3 border-bottom justify-content-between">
                        ...
                        <select class="form-select form-select-sm w-auto" id="revenueyear" data-url="<?php echo e(URL::to('/admin/dashboard')); ?>" style="border-radius:10px">
                            ...
                            <?php $__empty_1 = true; $__currentLoopData = $revenue_years; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $revenue): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <option value="<?php echo e($revenue->year); ?>" <?php echo e(date('Y') == $revenue->year ? 'selected' : ''); ?>><?php echo e($revenue->year); ?></option>
            ...
            <div class="card dash-chart-card h-100">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center justify-content-between mb-3 pb-3 border-bottom">
                        ...
                        <select class="form-select form-select-sm w-auto" id="doughnutyear" data-url="<?php echo e(request()->url()); ?>" style="border-radius:10px">
        ...
        <div class="row g-3 mb-4">
            <div class="col-xl-6" id="tour-products">
                <div class="card dash-table-card h-100">
                    <div class="card-body p-4">
                        <h5 class="dash-chart-title pb-3 color-changer border-bottom"><?php echo e(trans('labels.top_products')); ?></h5>
                                    ...
                                    <?php if(count($topitems) > 0): ?>
                                        <?php $__currentLoopData = @$topitems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <tr class="fs-7 fw-500 text-dark align-middle">
                                                <td><img src="<?php echo e(Helper::image_path($row['product_image']->image)); ?>" class="rounded hw-50 object" alt="" style="border-radius:10px !important"></td>
                                                <td><a href="<?php echo e(URL::to('admin/products/edit-' . $row->slug)); ?>" class="td_a"><?php echo e($row->item_name); ?></a></td>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="progress h-10-px" style="border-radius:10px"><div class="progress-bar <?php echo e($ran[array_rand($ran, 1)]); ?>" style="width:<?php echo e($per); ?>%;border-radius:10px" role="progressbar"><span class="sr-only"><?php echo e($per); ?>%</span></div></div>
                                                </td>
                                            </tr>
            ...
            <div class="col-xl-6">
                <div class="card dash-table-card h-100">
                    <div class="card-body p-4">
                        <h5 class="dash-chart-title pb-3 color-changer border-bottom"><?php echo e(trans('labels.top_customers')); ?></h5>
                        <div class="table-responsive">
                            <table class="table">
                                <thead><tr>
                                    <th class="fs-15 fw-500"><?php echo e(trans('labels.image')); ?></th>
                                ...
                                <tbody>
                                    ...
                                    <?php if(count($topusers) > 0): ?>
                                        <?php $__currentLoopData = @$topusers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <tr class="fs-7 fw-500 text-dark align-middle">
                                                <td><img src="<?php echo e(Helper::image_path($user->profile_image)); ?>" class="rounded hw-50 object" alt="" style="border-radius:10px !important"></td>
                                                <td><div class="fs-7 fw-500 td_a"><p><?php echo e($user->name); ?></p><p><?php echo e($user->mobile); ?></p></div></td>
    ...
    <div id="tourOverlay" style="display:none;position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,0.65);transition:opacity .3s"></div>
    <div id="tourTooltip" style="display:none;position:fixed;z-index:10001;background:#fff;border-radius:16px;padding:1.25rem 1.5rem;box-shadow:0 20px 60px rgba(0,0,0,0.25);max-width:340px;min-width:280px"></div>
...
<?php $__env->startSection('scripts'); ?>
    ...
    (function(){
        const steps = [
            { el: '#tour-stat-1', title: 'إحصائيات المتجر', desc: 'هنا تقدر تشوف إحصائيات متجرك - عدد المنتجات والطلبات والإيرادات.', position: 'bottom' },
            { el: '#tour-quick-add', title: 'إضافة سريعة', desc: 'من هنا تقدر تضيف منتجات وأقسام وإعدادات بسرعة بدون ما تدور في القوائم.', position: 'bottom' },
            { el: '#tour-chart', title: 'تقارير الإيرادات', desc: 'رسم بياني يوضح لك إيرادات متجرك على مدار السنة. تقدر تختار سنة مختلفة.', position: 'bottom' },
            { el: '#tour-orders-table', title: 'الطلبات الجارية', desc: 'جدول بالطلبات اللي محتاجة متابعة. تقدر تغير حالة الطلب من هنا.', position: 'top' },
            { el: '.sidebar', title: 'القائمة الجانبية', desc: 'من القائمة الجانبية تقدر تتنقل بين كل أقسام لوحة التحكم.', position: 'right' }
        ];
        let idx = 0;
        ...
        function show() {
            ...
            if (idx >= steps.length) { window.endTour(); return; }
            ...
            setTimeout(function() {
                ...
                tip.innerHTML =
                    '<div style="font-size:1rem;font-weight:800;color:#1e293b;margin-bottom:.35rem">' + s.title + '</div>' +
                    '<div style="font-size:.82rem;color:#64748b;margin-bottom:1rem;line-height:1.6">' + s.desc + '</div>' +
                    '<div style="display:flex;justify-content:space-between;align-items:center">' +
                        '<span style="font-size:.7rem;color:#94a3b8;font-weight:600">' + (idx+1) + ' / ' + steps.length + '</span>' +
                        '<div style="display:flex;gap:6px">' +
                            ...
                            (idx > 0 ? '<button onclick="window.prevTourStep()" style="padding:6px 12px;border-radius:8px;font-size:.82rem;font-weight:700;border:none;background:#f1f5f9;color:#64748b;cursor:pointer">السابق</button>' : '') +
                            '<button onclick="window.nextTourStep()" style="padding:6px 14px;border-radius:8px;font-size:.82rem;font-weight:700;border:none;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;cursor:pointer">' + (idx === steps.length - 1 ? 'إنهاء ✓' : 'التالي ←') + '</button>' +
                        '</div>' +
                    '</div>';
            }, 500);
        }

        window.nextTourStep = function() { idx++; show(); };
        window.prevTourStep = function() { if(idx > 0) { idx--; show(); } };
